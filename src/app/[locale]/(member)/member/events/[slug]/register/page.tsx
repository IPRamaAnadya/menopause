    ...
    try {
      ...
      if (!event.is_paid || finalPrice === 0) {
        ...
        if (!response.ok || !result.success) {
          ...
          // Show specific message for already registered with profile link
          if (errorMessage === 'Already registered') {
            throw new Error(t('registration.alreadyRegisteredMember'));
          }
        ...
        router.push(`/member/events/${resolvedParams.slug}/register/success?registration=${result.data.public_id}`);
      } else {
        ...
        if (result.data.url || result.data.payment_url) {
          ...
          router.push(`/member/events/${resolvedParams.slug}/register/success?registration=${publicId}`);
        } else {
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="mx-auto max-w-4xl px-4 py-12">
        {/* Back Button */}
        <Button variant="ghost" size="sm" asChild className="mb-8">
          <Link href={`/member/events/${resolvedParams.slug}`}>
            <ArrowLeft className="h-4 w-4 mr-2" />
